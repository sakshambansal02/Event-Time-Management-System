...
struct Time elapsed_time(struct Event e1, struct Event e2) {
  ...
  // values to put into the result Time
   struct Time ret;
  int calcSec =
abs((totalFromHMS(e1.st.hour,e1.st.min,e1.st.sec)-totalFromHMS(e2.st.hour,e2.st.min,e2.st.sec)));
 ret.hour = totalToHours(calcSec);
 ret.min = totalToMinutes(calcSec);
 ret.sec = totalToSeconds(calcSec);
 return ret;
